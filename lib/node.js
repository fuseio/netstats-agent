...
var BRIDGE_VERSION = process.env.BRIDGE_VERSION;
var ROLE = process.env.ROLE;
var FUSE_APP_VERSION = process.env.FUSE_APP_VERSION;
var PARITY_VERSION = process.env.PARITY_VERSION;
var NETSTATS_VERSION = process.env.NETSTATS_VERSION;
...
function Node ()
{
	this.info = {
		...
		role: ROLE || '',
		fuseAppVersion: FUSE_APP_VERSION || '',
		netStatsVersion: NETSTATS_VERSION || '',
...
Node.prototype.getStats = function(forced)
{
	...
	if (this._web3 && (lastFetchAgo >= UPDATE_INTERVAL || forced === true))
	{
		...
		async.parallel({
			...
			{
				// web3.eth.getMining(callback); // Not relevant for Aura
				callback(null, false);
			},
			hashrate: function (callback)
			{
				// web3.eth.getHashrate(callback); // Not relevant for Aura
				callback(null, '0x0');
			},
